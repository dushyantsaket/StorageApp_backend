...
export const login = async (req, res, next) => {
  ...
  const allSessionKeys = await getSessionKeysByUserId(user.id);

  if (allSessionKeys.length >= 2) {
    await redisClient.del(allSessionKeys[0]);
  }

  const sessionId = crypto.randomUUID();
  const redisKey = `session:${sessionId}`;
  await setSession(redisKey, {
    userId: user._id,
...
export const logoutAll = async (req, res) => {
  const { sid } = req.signedCookies;
  const session = await getSession(`session:${sid}`);
  const allSessionKeys = await getSessionKeysByUserId(session.userId);
